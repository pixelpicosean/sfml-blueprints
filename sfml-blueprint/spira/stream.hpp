...
namespace spira {
  ...
  class stream {
    public:
      ...
      // Stream operations
      stream<T> skipDuplicates(std::function<bool(T, T)> comparator = [](const T& a, const T& b) { return a == b; });
      stream<T> clone();
      stream<T> merge(stream<T>& other);
      stream<T> filter(std::function<bool(T)> filter = [](const T& v) { return (bool)v; });
      stream<T> filterBy(stream<bool>& otherObs);
      stream<T> scan(const std::function<T(T, T)> scan, const T& seed);
      template<typename U>
      stream<U> map(std::function<U(const T&)> map);
      template<typename U>
      stream<std::pair<T, U> > combine(stream<U>& other, SAMPLED_BY flag);

      // For binding side-effects
      void onValue(const std::function<void(T)> function);
      void offValue(const std::function<void(T)> function);
    private:
