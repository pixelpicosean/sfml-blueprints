...
namespace spira {
  ...
  template<typename T>
  stream<T> stream<T>::skipDuplicates(std::function<bool(T, T)> comparator) {
    stream<T> ret;
    this->hook([=](T value){ if (!comparator(ret.pimpl->value, value)) ret.push(value); });
    return ret;
  }
  ...
  template<typename T>
  stream<T> stream<T>::filterBy(stream<bool>& otherObs) {
    stream<T> ret;
    this->hook([=](T value){if(otherObs.pimpl->value) ret.push(value);});
    return ret;
  }

  template<typename T>
  stream<T> stream<T>::scan(const std::function<T(T, T)> scan, const T& seed) {
    stream<T> ret;
  ...
  template<typename T>
  template<typename U>
  stream<U> stream<T>::map(std::function<U(const T&)> map) {
    stream<U> ret;
  ...
  template<typename T>
  void stream<T>::onValue(const std::function<void(T)> function) {
